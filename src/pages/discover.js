...
function updateControls() {
  document.querySelectorAll('.view-btn').forEach(btn => {
    ...
    btn.innerHTML = btn.dataset.view.charAt(0).toUpperCase() + btn.dataset.view.slice(1) + (active ? ' <span class="active-dot w-1.5 h-1.5 bg-lime-500 shadow-[0_0_6px_var(--color-lime-500)] block rounded-full mb-[7px]"></span>' : '')
  ...
  document.querySelectorAll('.sort-btn').forEach(btn => {
    ...
    btn.innerHTML = `<span class="active-dot w-1.5 h-1.5 block rounded-full mb-[7px] ${active ? 'bg-lime-500 shadow-[0_0_6px_var(--color-lime-500)]' : 'bg-gray-300'}"></span>` + label
    btn.classList.toggle('text-gray-800', active)
    btn.classList.toggle('font-semibold', active)
  })
