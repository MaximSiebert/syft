...
export async function initAddItemForm({ defaultListId, onItemAdded, onListCreated, getListItems } = {}) {
  ...
  document.body.insertAdjacentHTML('beforeend', `
    <form id="add-item-form" class="fixed bottom-0 lg:right-8 right-4 lg:left-8 left-4 pb-3 z-20 bg-gray-50 sm:rounded-t-[25px] rounded-t-md"${shouldAnimate ? ' style="transform:translateY(100%)"' : ''}>
      <div class="relative shadow-lg bg-white sm:rounded-full rounded-md flex flex-wrap items-center border border-gray-200 hover:border-gray-300 focus-within:border-gray-300 transition-colors group">
        <div class="relative grow sm:w-auto w-full before:right-0 before:absolute before:w-[1px] before:h-5 before:transition-colors group-hover:before:bg-gray-300 focus-within:before:bg-gray-300 before:bg-gray-200 before:sm:block before:hidden before:top-1/2 before:transform before:-translate-y-1/2">
          <input type="text" id="add-item-input" placeholder="Paste a URL or write something short..." required
            class="w-full text-ellipsis bg-transparent sm:pl-4 px-3 py-3 text-sm transition-colors h-12 sm:border-0 border-b border-gray-200 group-hover:border-gray-300 outline-none placeholder:text-gray-500">
        ...
        <div class="sm:relative sm:min-w-72 sm:grow-0 grow text-ellipsis">
          <button type="button" id="list-picker-btn" class="relative w-full bg-transparent sm:pl-4 pl-3 pr-3 py-3 text-sm text-left text-gray-500 hover:text-gray-800 transition-colors h-12 outline-none truncate cursor-pointer flex items-center gap-2">
          ...
          <div id="list-picker-dropdown" class="group hidden absolute bottom-full mb-[1px] pb-1 sm:bg-transparent bg-gray-50 rounded-t-md left-[-1px] sm:right-auto right-[-1px] sm:w-full">
            <div class="bg-white border border-gray-200 group-hover:border-gray-300 transition-colors rounded-md overflow-hidden">
              ...
              <div id="list-picker-items" class="overflow-y-auto scrollbar-none max-h-[196px] py-2"></div>
