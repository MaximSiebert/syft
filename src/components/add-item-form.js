    ...
    <form id="add-item-form" class="fixed bottom-0 lg:right-8 right-4 lg:left-8 left-4 pb-3 z-20 bg-gray-50 sm:rounded-t-[25px] rounded-t-md"${shouldAnimate ? ' style="transform:translateY(100%)"' : ''}>
      <div class="relative shadow-lg bg-white sm:rounded-full rounded-md flex flex-wrap items-center border border-gray-200 hover:border-gray-300 transition-colors group">
        ...
        <div class="sm:relative sm:min-w-72 sm:grow-0 grow text-ellipsis">
          <button type="button" id="list-picker-btn" class="relative before:left-0 before:absolute before:w-[1px] before:h-5 before:transition-colors group-hover:before:bg-gray-300 before:bg-gray-200 before:sm:block before:hidden w-full bg-transparent pl-4 pr-3 py-3 text-sm text-left transition-colors h-12 outline-none truncate cursor-pointer flex items-center gap-2">
            <span id="list-picker-label" class="truncate w-full">Select list</span>
            <svg id="list-picker-arrow" class="shrink-0 transition-transform duration-200 justify-self-end" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="-0.6 -0.6 12 12" height="12" width="12">
